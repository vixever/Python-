...
letter_codes = [ord(ch) for ch in 'WASDRQwasdrq']    #ord把字符串转化为对应的数字
actions = ['up','left','down','right','restart','exit']
action_dict = dict(zip(letter_codes,actions*2))    #把字母与动作对应起来
    ...
    def move(self, direction):
        def move_row_left(row):
            ...
            def merge(row):
                ...
                return new_row
            return tighten(merge(tighten(row)))    #挤到一块再合并再挤到一块
